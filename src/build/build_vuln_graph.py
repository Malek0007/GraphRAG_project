...
# INPUT / OUTPUT
# ----------------------------
IN_FILE = "data/raw/nvd_cves.json"
OUT_FULL_JSON = "data/graphrag/vulnerabilities/vuln_graph_graphrag.json"
OUT_EXAMPLE_JSON = "data/graphrag/vulnerabilities/vuln_example_graph.json"
OUT_EXAMPLE_HTML = "data/graphrag/vulnerabilities/vuln_example_graph.html"
...
def add_edge(edges, src, rel, dst, props=None):
    e = {"source": src, "type": rel, "target": dst}
    if props:
        e["properties"] = props
    edges.append(e)
...
for item in vulns:
    ...
    for r in cve.get("references", []) or []:
        ...
        add_edge(edges, cve_node_id, "has_reference", url_node_id, {
            "tags": r.get("tags", [])
        })
    ...
    for key in ["cvssMetricV40", "cvssMetricV31", "cvssMetricV2"]:
        for entry in metrics.get(key, []) or []:
            ...
            add_edge(edges, cve_node_id, "has_cvss", cvss_node_id, {
                ...
                "metric_entry_type": entry.get("type")
            })
...
full_graph = {
    "meta": {
        "name": "vuln_graph_graphrag",
        "generated_at": datetime.utcnow().isoformat() + "Z",
        "description": "GraphRAG-ready NVD vulnerability graph with CVE-CWE-CPE-URL-CVSS and optional vendor/product/version, severity/vector/date/source nodes.",
...
ensure_parent_dir(OUT_FULL_JSON)
...
# include nodes that are targets from those CVEs
for e in edges:
    if e["source"] in example_node_ids:
        example_node_ids.add(e["target"])

# also include edges among included nodes
final_example_edges = []
for e in edges:
...
with open(OUT_EXAMPLE_JSON, "w", encoding="utf-8") as f:
    json.dump(example_graph, f, indent=2, ensure_ascii=False)
...
# ----------------------------
graph_str = json.dumps(example_graph, ensure_ascii=False)
...
<div class="panel">
  ...
  <div>Showing a small subgraph.</div>
  <div>Relations: <code>has_weakness</code>, <code>affects</code>, <code>has_reference</code>, <code>has_cvss</code>,
  <code>has_vendor</code>, <code>has_product</code>, <code>has_version</code>,
  <code>has_severity</code>, <code>has_vector</code>, <code>published_on</code>, <code>modified_on</code>, <code>reported_by</code>.</div>
</div>
<div id="cy"></div>
